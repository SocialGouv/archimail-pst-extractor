...
export class PSTNodeInputStream {
    ...
    constructor(
        pstFile: PSTFile,
        arg?: Buffer | OffsetIndexItem | PSTDescriptorItem | PSTDescriptorItem,
        encrypted?: boolean
    ) {
        ...
        if (arg instanceof OffsetIndexItem) {
            ...
            this.loadFromOffsetItem(
                pstFile.getOffsetIndexNode(
                    long.fromNumber(arg.offsetIndexIdentifier)
                )
            );
            ...
            if (encrypted !== undefined) {
                ...
                this._encrypted =
                    pstFile.encryptionType ==
                    PSTFile.ENCRYPTION_TYPE_COMPRESSIBLE;
            }
        }
        this.currentBlock = 0;
    ...
    public readSingleByte(): number {
        ...
        // get the next byte.
        const pos = item.fileOffset
            .add(this.currentLocation)
            .subtract(skipPoint);
    ...
    public readBlock(output: Buffer): number {
        ...
        while (!filled) {
            ...
            if (
                bytesRemaining >
                this.length.subtract(this.currentLocation).toNumber()
            ) {
    ...
    public seek(location: long): void {
        ...
        if (location.greaterThan(this.length)) {
            throw new Error(
                `PSTNodeInputStream::seek Attempt to seek past end of item! size = ${this.length}, seeking to:${location}`
            );
        }
        ...
        if (this.allData === null) {
            ...
            const newFilePos: long = blockStart
                .add(location)
                .subtract(skipPoint);
        ...
        try {
            if (this.read() === 0x78 && this.read() === 0x9c) {
                ...
                if (this.indexItems.length > 1) {
                    ...
                    multiStreams =
                        this.pstFile.read() === 0x78 &&
                        this.pstFile.read() === 0x9c;
                }
                ...
                if (multiStreams) {
                    ...
                    for (const i of this.indexItems) {
                        ...
                        outputStream = outputStream
                            ? Buffer.concat([outputStream, buf])
                            : buf;
                    }
                    this.indexItems = [];
                    this.skipPoints = [];
    ...
    private loadFromOffsetItem(offsetItem: OffsetIndexItem): void {
        ...
        if (bInternal) {
            ...
            if (data[0] === 0x1) {
                ...
                this._length = PSTUtil.convertLittleEndianBytesToLong(
                    data,
                    4,
                    8
                );
