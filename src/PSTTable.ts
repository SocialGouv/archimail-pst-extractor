...
export abstract class PSTTable {
    ...
    private readonly pstNodeInputStream: PSTNodeInputStream;

    private readonly subNodeDescriptorItems: Map<number, PSTDescriptorItem> =
        new Map();
    ...
    constructor(
        ...
        switch (
            ...
            default:
                throw new Error(
                    `PSTTable::constructor Unable to parse table, bad table type.  Unknown identifier: 0x${headdata[3].toString(
                        16
                    )}`
                );
        }
        ...
        if (headerByte !== 0xb5) {
            ...
            throw new Error(
                `PSTTable::constructor Unable to parse table, can't find BTHHEADER header information: ${headerByte}`
            );
        }
        ...
        this.numberOfIndexLevels =
            headerNodeInfo.pstNodeInputStream.read() & 0xff;
    ...
    public getNodeInfo(hnid: number): NodeInfo | null {
        ...
        if (this.subNodeDescriptorItems.has(hnid)) {
            ...
            try {
                ...
                subNodeInfo = new NodeInfo(
                    0,
                    subNodeIn.length.toNumber(),
                    subNodeIn
                );
            } catch (err: unknown) {
                throw new Error(
        ...
        if (whichBlock > this.arrayBlocks.length) {
            throw new Error(
                `PSTTable::getNodeInfo: block doesn't exist: ${hnid}, ${whichBlock}, ${this.arrayBlocks.length}`
            );
        }
